...
def _worker_init(
    ...
    # Set up logging in the worker process so evaluator logs are visible.
    #
    # Under fork(), the worker inherits the parent's root logger handlers —
    # including the controller's FileHandler, whose file descriptor is shared
    # across processes. Writing to that FD from multiple processes would
    # interleave bytes mid-line. We therefore:
    #   1. strip every inherited handler from the worker's root logger, and
    #   2. attach a single QueueHandler that ships LogRecords back to the
    #      main process, which owns the *real* handlers. A QueueListener
    #      thread in the main process drains the queue and dispatches to
    #      the file/console handlers (thread-safe via Handler.lock).
    #
    # If no queue was passed (legacy callers), fall back to a plain stderr
    # handler so worker logs are at least visible on the console.
    ...
    root.setLevel(getattr(_logging, _worker_config.log_level, _logging.INFO))

    if log_queue is not None:
        root.addHandler(_QueueHandler(log_queue))
    else:
...
class ProcessParallelController:
    ...
    def __init__(
        ...
        # Cross-process logging plumbing: workers push LogRecords into
        # self._log_queue, and self._log_listener (a thread in the main
        # process) drains the queue and dispatches records to whatever
        # handlers the main process's root logger currently has.
    ...
    def start(self) -> None:
        ...
        # Determine the multiprocessing context up front so the log queue
        # and the process pool share the same start method. Mixing contexts
        # (e.g., a fork-context Queue with a spawn-context pool) can
        # deadlock or silently drop records.
        use_spawn_fallback = (
            sys.version_info < (3, 11) and self.config.max_tasks_per_child is not None
        )
        mp_ctx = mp.get_context("spawn") if use_spawn_fallback else mp.get_context()

        # ── Cross-process logging setup ──────────────────────────────────
        # Start a QueueListener *in the main process* that drains records
        # emitted by worker QueueHandlers and forwards them to whatever
        # handlers are currently attached to the root logger (typically the
        # controller's FileHandler + StreamHandler). This is the only way
        # to get workers' logs into the controller's log file without
        # racing on a shared file descriptor.
        if self.log_file is not None and self._log_listener is None:
            root_handlers = list(logging.getLogger().handlers)
            if root_handlers:
                self._log_queue = mp_ctx.Queue(-1)
                self._log_listener = QueueListener(
                    self._log_queue, *root_handlers, respect_handler_level=True
                )
                self._log_listener.start()
                logger.info(f"Cross-process log queue active → {self.log_file}")
            else:
                # No handlers to dispatch to — don't bother spinning up the
                # listener; workers will fall back to stderr.
    ...
    def stop(self) -> None:
        ...
        # Tear down cross-process logging AFTER workers have exited, so any
        # in-flight records have a chance to make it through the queue.

...
class OpenAILLM(LLMInterface):
    ...
    async def _call_api(self, params: Dict[str, Any]) -> str:
        ...
        if not response.choices:
            raise RuntimeError(
                f"LLM returned empty choices. This usually means the endpoint "
                f"({self.api_base}) is not OpenAI-compatible. "
                f"For AWS Bedrock, set provider: bedrock in your config."
            )
